...
void packing(uint32_t* out, const uint8_t* in) {
		...
		LE_LOAD(out + 2, in + 4);
		LE_LOAD(out + 3, in + 12);
		SWAPMOVEt(out[0], out[0], 0x0a0a0a0a, 3);
		SWAPMOVEt(out[1], out[1], 0x0a0a0a0a, 3);
		SWAPMOVEt(out[2], out[2], 0x0a0a0a0a, 3);
		SWAPMOVEt(out[3], out[3], 0x0a0a0a0a, 3);
		SWAPMOVEt(out[2], out[0], 0x30303030, 2);
		SWAPMOVEt(out[1], out[0], 0x0c0c0c0c, 4);
		SWAPMOVEt(out[3], out[0], 0x03030303, 6);
		SWAPMOVEt(out[1], out[2], 0x0c0c0c0c, 2);
		SWAPMOVEt(out[3], out[2], 0x03030303, 4);
		SWAPMOVEt(out[3], out[1], 0x03030303, 2);
#else
	uint32_t tmp;
	LE_LOAD(out, in);
	LE_LOAD(out + 1, in + 8);
...
void permute_tk(uint32_t* tk, const uint8_t* key, const int rounds) {
	...
	packing(tk1, key);
	memcpy(tmp, tk, 16);
	XOR2(tmp[0],tmp[0] ,tk1[0],tmp[1],tmp[1],tk1[1]);
	XOR2(tmp[2],tmp[2] ,tk1[2],tmp[3],tmp[3],tk1[3]);
...
void tk_schedule_23(uint32_t* rtk_23, const uint8_t* tk2, const uint8_t * tk3) {
	...
	for(int i = 0; i < SKINNY128_384_ROUNDS; i++) {				// add rconsts
			for(int j = 0; j < 4; j=j+2){
				XOR2(rtk_23[i*4+j],rtk_23[i*4+j],rconst_32_bs[i*4+j],
					 rtk_23[i*4+j+1],rtk_23[i*4+j+1],rconst_32_bs[i*4+j+1]);
