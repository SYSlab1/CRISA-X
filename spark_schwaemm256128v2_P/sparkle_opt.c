...
void sparkle_opt(uint32_t *state, int brans, int steps)
{
  ...
  uint32_t rc, tmpx, tmpy, x0, y0,tmp_v,zero=0;
  ...
  for(i = 0; i < steps; i ++) {
    ...
    for(j = 0; j < 2*brans; j += 2) {
      rc = RCON[j>>1];
#ifdef SET_TIE_FUNC
      state[j] += ROTXOR(0,state[j+1],state[j+1], 31,1);
	  state[j+1] = ROTXOR( state[j+1],state[j],state[j], 24,8);
	  state[j] ^= rc;
	  state[j] += ROTXOR(0,state[j+1],state[j+1], 17,15);
	  state[j+1] = ROTXOR(state[j+1],state[j],state[j], 17,15);
	  state[j] ^= rc;
	  state[j] += state[j+1];
	  state[j+1] = ROTXOR(state[j+1],state[j],state[j], 31,1);
	  state[j] ^= rc;
	  state[j] += ROTXOR(0,state[j+1], state[j+1],24,8);
	  state[j+1] = ROTXOR(state[j+1],state[j],state[j], 16,16);
	  state[j] ^= rc;
#else
    ...
    for(j = 2; j < brans; j += 2) {
#ifdef SET_TIE_FUNC
    XOR2(tmpx,tmpx,state[j],tmpy,tmpy,state[j+1]);
#else
      tmpx ^= state[j];
      tmpy ^= state[j+1];
#endif
    ...
    tmpx = ROTXOR(0,SHIFTLXOR(tmpx,tmpx,16),SHIFTLXOR(tmpx,tmpx,16),16,16);
    tmpy = ROTXOR(0,SHIFTLXOR(tmpy,tmpy,16),SHIFTLXOR(tmpy,tmpy,16),16,16);


    for (j = 2; j < brans; j += 2) {
      state[j-2] = XOR5(state[j+brans] ,state[j] ,tmpy,zero,zero);
      state[j+brans] = state[j];
      state[j-1] = XOR5(state[j+brans+1] ,state[j+1] ,tmpx,zero,zero);
...
#endif
#ifdef SET_TIE_FUNC
    state[brans-2] = XOR5(state[brans],x0, tmpy,0,0);
	state[brans] = x0;
	state[brans-1] = XOR5(state[brans+1],y0,tmpx,0,0);
	state[brans+1] = y0;
