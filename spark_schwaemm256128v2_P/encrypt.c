...
#ifdef SET_TIE_FUNC
    state[i] = XOR5(zero,zero,state[j] ,in32[i] , state[RATE_WORDS+i]);
    state[j] = XOR5(zero,state[j],tmp,in32[j],state[RATE_WORDS+CAP_INDEX(j)]);
...
static void rho_whi_aut_last(uint32_t *state, const uint8_t *in, size_t inlen)
{
  ...
  for (i = 0, j = RATE_WORDS/2; i < RATE_WORDS/2; i++, j++) {
    tmp = state[i];
#ifdef SET_TIE_FUNC
    state[i] = XOR5(zero,zero,state[j] ,buffer[i] , state[RATE_WORDS+i]);
    state[j] = XOR5(state[j],tmp,buffer[j],buffer[j],state[RATE_WORDS+CAP_INDEX(j)]);
...
#ifdef SET_TIE_FUNC
    state[i] = XOR5(zero,zero,state[j] ,in32[i] , state[RATE_WORDS+i]);
    state[j] = XOR5(zero,state[j],tmp1,in32[j],state[RATE_WORDS+CAP_INDEX(j)]);
    XOR2(out32[i],in32[i],tmp1,out32[j],in32[j],tmp2);
...
int crypto_aead_encrypt(UChar *c, ULLInt *clen, const UChar *m, ULLInt mlen, \
  ...
  const UChar *k)__attribute__((noinline))
{
