...
void precompute_rkeys_TIE(u32* rkey, const u8* key) {
    u32 tmp;
    //classical initialization
    rkey[0] = U32BIG(((u32*)key)[3]);
    rkey[1] = U32BIG(((u32*)key)[1]);
    ...
    for(int i = 0; i < 16; i+=2) {
        rkey[i+4] = rkey[i+1];
        rkey[i+5] = KEY_UPDATE_TIE(rkey[i]);
    }
    // transposition to fixsliced representations
    for(int i = 0; i < 20; i+=10) {
        REARRANGE_RKEY_0_TIE(rkey[i]);
        REARRANGE_RKEY_0_TIE(rkey[i + 1]);
        REARRANGE_RKEY_1_TIE(rkey[i + 2]);
        REARRANGE_RKEY_1_TIE(rkey[i + 3]);
        REARRANGE_RKEY_2_TIE(rkey[i + 4]);
        REARRANGE_RKEY_2_TIE(rkey[i + 5]);
        REARRANGE_RKEY_3_TIE(rkey[i + 6]);
        REARRANGE_RKEY_3_TIE(rkey[i + 7]);
    }
    // keyschedule according to fixsliced representations
    for(int i = 20; i < 80; i+=10) {
        rkey[i-19];
        KEY_TRIPLE_UPDATE_0_TIE(rkey[i-20]);
        KEY_DOUBLE_UPDATE_1_TIE(rkey[i-17]);
        KEY_TRIPLE_UPDATE_1_TIE(rkey[i-18]);
        KEY_DOUBLE_UPDATE_2_TIE(rkey[i-15]);
        KEY_TRIPLE_UPDATE_2_TIE(rkey[i-16]);
        KEY_DOUBLE_UPDATE_3_TIE(rkey[i-13]);
        KEY_TRIPLE_UPDATE_3_TIE(rkey[i-14]);
        KEY_DOUBLE_UPDATE_4_TIE(rkey[i-11]);
        KEY_TRIPLE_UPDATE_4_TIE(rkey[i-12]);
        SWAPMOVE_TIE(rkey[i], rkey[i], 0x00003333, 16);
        SWAPMOVE_TIE(rkey[i], rkey[i], 0x55554444, 1);
        SWAPMOVE_TIE(rkey[i+1], rkey[i+1], 0x55551100, 1);
    }
}
...
*****************************************************************************/
void giftb128_TIE(u8* ctext, const u8* ptext, const u32* rkey)__attribute__ ((noinline)) {
    u32 tmp, state[4];
    state[0] = U32BIG(((u32*)ptext)[0]);
    state[1] = U32BIG(((u32*)ptext)[1]);
    state[2] = U32BIG(((u32*)ptext)[2]);
    state[3] = U32BIG(((u32*)ptext)[3]);
    QUINTUPLE_ROUND_TIE(state, rkey, rconst);
    QUINTUPLE_ROUND_TIE(state, rkey + 10, rconst + 5);
    QUINTUPLE_ROUND_TIE(state, rkey + 20, rconst + 10);
    QUINTUPLE_ROUND_TIE(state, rkey + 30, rconst + 15);
    QUINTUPLE_ROUND_TIE(state, rkey + 40, rconst + 20);
    QUINTUPLE_ROUND_TIE(state, rkey + 50, rconst + 25);
    QUINTUPLE_ROUND_TIE(state, rkey + 60, rconst + 30);
    QUINTUPLE_ROUND_TIE(state, rkey + 70, rconst + 35);
    U8BIG(ctext, state[0]);
...
void giftb128(u8* ctext, const u8* ptext, const u32* rkey)__attribute__ ((noinline)) {
    u32 tmp, state[4];
    state[0] = U32BIG(((u32*)ptext)[0]);
    state[1] = U32BIG(((u32*)ptext)[1]);
    state[2] = U32BIG(((u32*)ptext)[2]);
