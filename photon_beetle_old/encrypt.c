	...
	const uint8_t  Constant)
{
	...
	///State_inout[STATE_INBYTES - 1] = ShiftRXor8(State_inout[STATE_INBYTES - 1],Constant,LAST_THREE_BITS_OFFSET);
	//not helping
	//printf ("\n%hhx",State_inout[STATE_INBYTES - 1]);
...
/*	for (i = 0; i < iolen_inbytes - 1; i++)
		{
			out[i] = ROT8(in[i],(in[(i+1)] & 1),1,7);
		}
		out[iolen_inbytes - 1] = ROT8(in[i],(tmp & 1),1,7);
*/
