...
// SBox
#define SBox_TIE(x0, x1, x2, x3, t0, t1) \
do {\
	CPY(t0, x2    ); \
	XORAND2(x1,x2,t0,x1);\
	XOR(x3, x3, t0); \
	...
	CPY(x2, x3    ); \
	OR (t1, t1, x0); \
	XOR2(x0,x0,t0,x1,x1,x0);\
	XOROR2(x2,x1,x2,t0);\
	XOR2(x1,x1,t1,x3,x3,x1);\
} while(0) ;
...
# define mul2_GF16_0x13_xor_TIE(x0, x1, x2, x3, sum0, sum1, sum2, sum3, t0, t1, t2, t3) \
do { \
XOR2(t3, x3, x0,sum0, sum0, x0); /* Output : ( MSB ) x1 ,x2 ,x3 , x0 ( LSB ) */ \
XOR2(sum1, sum1, t3,sum2, sum2, x2);\
XOR(sum3, sum3, x1); \
} while(0) ;
...
# define mul4_GF16_0x13_xor_TIE(x0, x1, x2, x3, sum0, sum1, sum2, sum3, t0, t1, t2, t3) \
do { \
XOR2(t3, x3, x0,t0, x0, x1); /* Output : ( MSB ) x2 ,x3 ,x0 , x1 ( LSB ) */ \
XOR2(sum0, sum0, x1,sum1, sum1, t0); \
XOR2(sum2, sum2, t3,sum3, sum3, x2); \
} while(0) ;

# define mul5_GF16_0x13_xor_TIE(x0, x1, x2, x3, sum0, sum1, sum2, sum3, t0, t1, t2, t3) \
do { \
XOR2(t2, x2 ,x0,t3, x3 ,x1); \
XOR2(t1, x1 ,t2,t0, x0 ,t3); /* Output : ( MSB ) x2 ,x0 ,x1 , x3 ( LSB ) */ \
XOR2(sum0, sum0, t3,sum1, sum1, t1); \
XOR2(sum2, sum2, t0,sum3, sum3, t2); \
} while(0) ;

# define mul6_GF16_0x13_xor_TIE(x0, x1, x2, x3, sum0, sum1, sum2, sum3, t0, t1, t2, t3) \
do { \
XOR2(t3, x3 ,x1,t1, x1 ,x0); \
XOR2(t2, x2 ,t1,t0, x0 ,t2); \
XOR2(t2, t2 ,t3,sum0, sum0, t1);  /* Output : ( MSB ) x0 ,x2 ,x3 , x1 ( LSB ) */ \
XOR2(sum1, sum1, t3,sum2, sum2, t2); \
XOR(sum3, sum3, t0); \
} while(0) ;

# define mul8_GF16_0x13_xor_TIE(x0, x1, x2, x3, sum0, sum1, sum2, sum3, t0, t1, t2, t3) \
do { \
XOR2(t3, x3 ,x0,t0, x0 ,x1); \
XOR2(t1, x1 ,x2,sum0, sum0, x2); /* Output : ( MSB ) x3 ,x0 ,x1 , x2 ( LSB ) */ \
XOR2(sum1, sum1, t1,sum2, sum2, t0); \
XOR(sum3, sum3, t3); \
} while(0) ;

# define mulb_GF16_0x13_xor_TIE(x0, x1, x2, x3, sum0, sum1, sum2, sum3, t0, t1, t2, t3) \
do { \
XOR2(t2, x2 ,x0,t1, x1 ,x3); \
XOR2(t0, x0 ,t1,t3, x3 ,t2); /* Output : ( MSB ) x1 ,x2 ,x0 , x3 ( LSB ) */ \
XOR2(sum0, sum0, t3,sum1, sum1, t0); \
XOR2(sum2, sum2, t2,sum3, sum3, t1); \
} while(0)
...
do { \
    ...
    sum2 = 0; \
    sum3 = 0; \
    mul2_GF16_0x13_xor_TIE(x03, x02, x01, x00, sum0, sum1, sum2, sum3, t0, t1, t2, t3); \
    mul4_GF16_0x13_xor_TIE(x13, x12, x11, x10, sum0, sum1, sum2, sum3, t0, t1, t2, t3); \
    mul2_GF16_0x13_xor_TIE(x23, x22, x21, x20, sum0, sum1, sum2, sum3, t0, t1, t2, t3); \
    mulb_GF16_0x13_xor_TIE(x33, x32, x31, x30, sum0, sum1, sum2, sum3, t0, t1, t2, t3); \
    mul2_GF16_0x13_xor_TIE(x43, x42, x41, x40, sum0, sum1, sum2, sum3, t0, t1, t2, t3); \
    mul8_GF16_0x13_xor_TIE(x53, x52, x51, x50, sum0, sum1, sum2, sum3, t0, t1, t2, t3); \
    mul5_GF16_0x13_xor_TIE(x63, x62, x61, x60, sum0, sum1, sum2, sum3, t0, t1, t2, t3); \
    mul6_GF16_0x13_xor_TIE(x73, x72, x71, x70, sum0, sum1, sum2, sum3, t0, t1, t2, t3); \
} while(0) ;
...
#define AddKey_TIE(row0123_bit0, row0123_bit1, row0123_bit2, row0123_bit3, row4567_bit0, row4567_bit1, row4567_bit2, row4567_bit3, round) \
do { \
	XOR2(row0123_bit0,row0123_bit0,RC[round][0],row0123_bit1,row0123_bit1,RC[round][1]);\
    XOR2(row0123_bit2,row0123_bit2,RC[round][2],row0123_bit3,row0123_bit3,RC[round][3]);\
    XOR2(row4567_bit0,row4567_bit0,RC[round][4],row4567_bit1,row4567_bit1,RC[round][5]);\
    XOR2(row4567_bit2,row4567_bit2,RC[round][6],row4567_bit3,row4567_bit3,RC[round][7]);\
} while(0) ;

#define ROT8L_1_INPLACE_TIE(x) ((x) = ROTOR((x) ,(x),1,7))
#define ROT8L_2_INPLACE_TIE(x) ((x) = ROTOR((x) ,(x),2,6))
#define ROT8L_3_INPLACE_TIE(x) ((x) = ROTOR((x) ,(x),3,5))
#define ROT8L_4_INPLACE_TIE(x) ((x) = ROTOR((x) ,(x),4,4))
#define ROT8L_5_INPLACE_TIE(x) ((x) = ROTOR((x) ,(x),5,3))
#define ROT8L_6_INPLACE_TIE(x) ((x) = ROTOR((x) ,(x),6,2))
#define ROT8L_7_INPLACE_TIE(x) ((x) = ROTOR((x) ,(x),1,7))
...
#define ShiftRow_TIE_1(x0, x1, x2, x3) \
do { \
    ROT8L_1_INPLACE_TIE(x0); \
    ROT8L_1_INPLACE_TIE(x1); \
    ROT8L_1_INPLACE_TIE(x2); \
    ROT8L_1_INPLACE_TIE(x3); \
} while(0) ;
#define ShiftRow_TIE_2(x0, x1, x2, x3) \
do { \
    ROT8L_2_INPLACE_TIE(x0); \
    ROT8L_2_INPLACE_TIE(x1); \
    ROT8L_2_INPLACE_TIE(x2); \
    ROT8L_2_INPLACE_TIE(x3); \
} while(0) ;
#define ShiftRow_TIE_3(x0, x1, x2, x3) \
do { \
    ROT8L_3_INPLACE_TIE(x0); \
    ROT8L_3_INPLACE_TIE(x1); \
    ROT8L_3_INPLACE_TIE(x2); \
    ROT8L_3_INPLACE_TIE(x3); \
} while(0) ;
#define ShiftRow_TIE_4(x0, x1, x2, x3) \
do { \
    ROT8L_4_INPLACE_TIE(x0); \
    ROT8L_4_INPLACE_TIE(x1); \
    ROT8L_4_INPLACE_TIE(x2); \
    ROT8L_4_INPLACE_TIE(x3); \
} while(0) ;
#define ShiftRow_TIE_5(x0, x1, x2, x3) \
do { \
    ROT8L_5_INPLACE_TIE(x0); \
    ROT8L_5_INPLACE_TIE(x1); \
    ROT8L_5_INPLACE_TIE(x2); \
    ROT8L_5_INPLACE_TIE(x3); \
} while(0) ;
#define ShiftRow_TIE_6(x0, x1, x2, x3) \
do { \
    ROT8L_6_INPLACE_TIE(x0); \
    ROT8L_6_INPLACE_TIE(x1); \
    ROT8L_6_INPLACE_TIE(x2); \
    ROT8L_6_INPLACE_TIE(x3); \
} while(0) ;
#define ShiftRow_TIE_7(x0, x1, x2, x3) \
do { \
    ROT8L_7_INPLACE_TIE(x0); \
    ROT8L_7_INPLACE_TIE(x1); \
    ROT8L_7_INPLACE_TIE(x2); \
    ROT8L_7_INPLACE_TIE(x3); \
} while(0) ;
...
#define ExtractBytesWord_TIE(byte0, byte1, byte2, byte3, word0) \
do { \
    byte0 = (uint8_t)( word0 & 0xFF); \
    byte1 = (uint8_t)(SHIFTRXORAND(0,word0,0xFF,8));\
    byte2 = (uint8_t)(SHIFTRXORAND(0,word0,0xFF,16));\
    byte3 = (uint8_t)(SHIFTRXORAND(0,word0,0xFF,24));\
} while(0);
...
#define ExtractBytesWords_State_TIE(\
    x00, x01, x02, x03,  \
    x10, x11, x12, x13,  \
    x20, x21, x22, x23,  \
    x30, x31, x32, x33,  \
    ...
    row4567_nb0, row4567_nb1, row4567_nb2, row4567_nb3) \
do { \
    ExtractBytesWord_TIE(x00, x10, x20, x30, row0123_nb0) \
    ExtractBytesWord_TIE(x01, x11, x21, x31, row0123_nb1) \
    ExtractBytesWord_TIE(x02, x12, x22, x32, row0123_nb2) \
    ExtractBytesWord_TIE(x03, x13, x23, x33, row0123_nb3) \
    ExtractBytesWord_TIE(x40, x50, x60, x70, row4567_nb0) \
    ExtractBytesWord_TIE(x41, x51, x61, x71, row4567_nb1) \
    ExtractBytesWord_TIE(x42, x52, x62, x72, row4567_nb2) \
    ExtractBytesWord_TIE(x43, x53, x63, x73, row4567_nb3) \
} while(0);


#define ReorderWord_TIE(x, t) \
do { \
...
/*  s37 s36 s35 s34 s33 s32 s31 s30 s27 s26 s25 s24 s23 s22 s21 s20 s17 s16 s15 s14 s13 s12 s11 s10 s07 s06 s05 s04 s03 s02 s01 s00 */ \
    t = SHIFTRXORAND(x,x,0x0C0C0C0CU,2); x = XOR5(x,t,(t << 2U),0,0); \
...
/*  s37 s36 s33 s32 s35 s34 s31 s30 s27 s26 s23 s22 s25 s24 s21 s20 s17 s16 s13 s12 s15 s14 s11 s10 s07 s06 s03 s02 s05 s04 s01 s00 */ \
	t = SHIFTRXORAND(x,x,0x00F000F0U,4); x = XOR5(x,t,(t << 4U),0,0); \
...
/*  s37 s36 s33 s32 s27 s26 s23 s22 s35 s34 s31 s30 s25 s24 s21 s20 s17 s16 s13 s12 s07 s06 s03 s02 s15 s14 s11 s10 s05 s04 s01 s00 */ \
	t = SHIFTRXORAND(x,x,0x0000FF00U,8); x = XOR5(x,t,(t << 8U),0,0); \
...
/*  s37 s36 s33 s32 s27 s26 s23 s22 s17 s16 s13 s12 s07 s06 s03 s02 s35 s34 s31 s30 s25 s24 s21 s20 s15 s14 s11 s10 s05 s04 s01 s00 */ \
	t = SHIFTRXORAND(x,x,0x0000FF00U,1); x = XOR5(x,t,(t << 1U),0,0); \
...
/*  s37 s33 s36 s32 s27 s23 s26 s22 s17 s13 s16 s12 s07 s03 s06 s02 s35 s31 s34 s30 s25 s21 s24 s20 s15 s11 s14 s10 s05 s01 s04 s00 */ \
	t = SHIFTRXORAND(x,x,0x0C0C0C0CU,2); x = x = XOR5(x,t,(t << 2U),0,0); \
...
/*  s37 s33 s27 s23 s36 s32 s26 s22 s17 s13 s07 s03 s16 s12 s06 s02 s35 s31 s25 s21 s34 s30 s24 s20 s15 s11 s05 s01 s14 s10 s04 s00 */ \
	t = SHIFTRXORAND(x,x,0x00F000F0U,4); x = x = XOR5(x,t,(t << 4U),0,0); \
...
#define ReorderState_TIE(row0, row1, row2, row3, row4, row5, row6, row7, wtmp0, wtmp1) \
do { \
    row0 = ((uint32_t *)State_in)[0]; ReorderWord_TIE(row0, wtmp0); \
    row1 = ((uint32_t *)State_in)[1]; ReorderWord_TIE(row1, wtmp1); \
    row2 = ((uint32_t *)State_in)[2]; ReorderWord_TIE(row2, wtmp0); \
    row3 = ((uint32_t *)State_in)[3]; ReorderWord_TIE(row3, wtmp1); \
    row4 = ((uint32_t *)State_in)[4]; ReorderWord_TIE(row4, wtmp0); \
    row5 = ((uint32_t *)State_in)[5]; ReorderWord_TIE(row5, wtmp1); \
    row6 = ((uint32_t *)State_in)[6]; ReorderWord_TIE(row6, wtmp0); \
    row7 = ((uint32_t *)State_in)[7]; ReorderWord_TIE(row7, wtmp1); \
...
void PHOTON_Permutation(unsigned char *State_in)
{
...
#ifdef SET_TIE_GENRIC
    	ReorderState_TIE(w0, w1, w2, w3, w4, w5, w6, w7, wtmp0, wtmp1);
	...
	for(i = 0; i < ROUND; i++)
	{
...
#ifdef SET_TIE_GENRIC
		AddKey_TIE(w0, w1, w2, w3, w4, w5, w6, w7, i)
		SBox_TIE(w0, w1, w2, w3, wtmp0, wtmp1);
		SBox_TIE(w4, w5, w6, w7, wtmp0, wtmp1);
		ExtractBytesWords_State_TIE(
		        s00, s01, s02, s03,
		        s10, s11, s12, s13,
		        s20, s21, s22, s23,
		        s30, s31, s32, s33,
        ...
        w4, w5, w6, w7) 
     	...
     	ShiftRow_TIE_1(s10, s11, s12, s13);
        ShiftRow_TIE_2(s20, s21, s22, s23);
        ShiftRow_TIE_3(s30, s31, s32, s33);
        ShiftRow_TIE_4(s40, s41, s42, s43);
        ShiftRow_TIE_5(s50, s51, s52, s53);
        ShiftRow_TIE_6(s60, s61, s62, s63);
        ShiftRow_TIE_7(s70, s71, s72, s73);
